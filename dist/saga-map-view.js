...
export function renderSagaScene(root,level,atlases){
  const scene=root.getElementById('saga-scene'),place=level.id===1102?[level.location,'valley',38,'Piccolo · aliança com Goku']:level.id===1103?[level.location,'valley',75,'Goku · Piccolo · Gohan']:level.id===1201?['Terra · Local do confronto','valley',90,'Raditz · batalha em equipe']:level.id===1301?[level.location,'snake-way-map-v31',50,'Goz · Mez · guardião da passagem']:places[level.id];
  ...
  const atlas=level.id===1102?atlases.piccolo:[1103,1301].includes(level.id)?atlases.goku:level.saga==='freeza'?atlases.namekVillains:atlases.sagaEnemies;
