...
const img=await loadImage(fileURLToPath(new URL('../dist/assets/freeza-v20.png',import.meta.url)));
const atlas=build(img,6,4);assert.equal(atlas.frames.length,24);
const sheet=createCanvas(1440,720),ctx=sheet.getContext('2d');ctx.imageSmoothingEnabled=false;
for(let i=0;i<24;i++){
 ...
 ctx.fillStyle=i%2?'#132943':'#243551';ctx.fillRect(x,y,240,180);
...
fs.writeFileSync(new URL('../output/freeza-sprite-check.png',import.meta.url),sheet.toBuffer('image/png'));
console.log('24 boss poses rendered for visual inspection.',JSON.stringify(atlas.frames));
